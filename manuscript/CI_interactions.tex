...
\author{Alyssa R. Cirtwill$^{1\dagger}$, \&  Anna Ekl\"{o}f$^{1\ddagger}$, Tomas Roslin$^{2,3\star}$, Kate Wootton$^{2\diamond}$, Dominique Gravel$^{4\triangleright}$}
\date{\begin{minipage}[h]{0.45\textwidth}
...
\end{minipage}\hspace{.05\textwidth}\begin{minipage}[h]{0.45\textwidth}
\small$^3$ Spatial Foodweb Ecology Group\\ 
Department of Agricultural Sciences \\ 
PO Box 27 (Latokartanonkaari 5)\\ 
FI-00014 University of Helsinki, Finland\\
$^4$ D\'{e}partement de biologie\\ 
...
tel: +46 723 158464
\end{minipage}\hspace{.05\textwidth}\begin{minipage}[h]{0.22\textwidth}
\small$^\ddagger$ anna.eklof@liu.se\\
$^\star$ tomas.roslin@slu.se
\end{minipage}\hspace{.05\textwidth}\begin{minipage}[h]{0.38\textwidth}
\small$^\diamond$ kate.wootton@slu.se\\
...
AC and DG wrote the code. AC performed statistical analyses. All authors contributed to writing and revising the manuscript. TR contributed dat. [[Anything missing?]]
...
ecological networks; probabilistic interactions; Bayesian networks; sampling error; spatial variability; temporal variability
...
% AC: Good idea - I've also changed the "In this study, we ..." paragraph to make it sound less like we just built some software and hopefully emphasise the conceptual side more.
...
\section*{Introduction}
    ...
    % \citet{Eklof2013} also introduced variability  in the computation of network dimensions, allowing the absence of interactions within the n-dimension niche of every species. % AC: Does this really fit in now that this is in the introduction?
...
\section*{Estimating detection and process uncertainty}
    ...
    %AC: I've added this section back in but shortened it slightly (mostly the first paragraph). We're ok on word-count with this included.
...
\section*{A naive quantification of uncertainty}
  %AC: This section header was too long and running beyond a line... is the shortened version ok?
...
\section*{Scaling up to networks - an empirical example}
  ...
  \subsection*{Scaling up to network metrics}
    ...
    % AC: Added nestedness and replaced the Barbour data with training data from one site. Couldn't find any good published data.
