  ...
  \subsection*{A Bayesian framework for interaction probabilities}
    ...
    \subsubsection*{Choosing a prior distribution}  [[expand]]
      ...
      \textbf{Informed priors}
        ...
        See \emph{Appendix S3} for a simple quantitative example using degree distributions to obtain $\alpha$ and $\beta$.


      \textbf{Empirical priors} 

        As well as calculating uninformative priors or informed priors (by specifying $\alpha$ and $\beta$ or deriving them from data outside the study system), it is possible to estimate $\alpha$ and $\beta$ directly from the data being analysed. This is known as Empirical Bayes~\citep{Casella1985}. In empirical Bayes, we fit a beta distribution to interaction probabilities observed in the empirical data (or some portion thereof, where a pilot study has been performed) or calculated using some property of the empirical data (in the case study below, we use species' degree distributions). Empirical Bayes is used in medical testing~\citep{Copas1972,Spiegelhalter2000} and disease modelling~\citep{Leyland2005,Brooks2015}, and is easily applicable to ecological systems. It is most useful where researchers do not wish to use an uninformative prior but data from a sufficiently similar system are not available (as in our case study).


        Despite its practicality, empirical Bayes has been criticized for being overly subjective~\citep{Berger2006} and because estimating hyperparameters from data serves as an approximation of some true hyperparameters which remain unknown~\citep{}.

      [[using data from the data in question]][[contentious]][[should these be drawn from hyperpriors or fixed for each interaction]]
