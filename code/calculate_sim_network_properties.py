...
def web_reader(webdir,proportion,post_number,sample_number,nettype): 
  ...
  if sample_number==0:
    if nettype=="SG":
      ...
      f=open(webdir+'gp_posterior/P'+str(post_number)+'.web','r')
  else:
    if nettype=="SG":
      f=open(webdir+'detection_filter/'+str(proportion)+'/P'+str(post_number)+'_'+str(sample_number)+'.web','r')
    else:
      f=open(webdir+'gp_detection_filter/'+str(proportion)+'/P'+str(post_number)+'_'+str(sample_number)+'.web','r')
  for line in f:
    galler=line.split()[0]
    salix=line.split()[1]
    gallers.add(galler)
...
def write_summary_file(summary_dict,nettype):
  f=open('../data/randomised_webs/'+nettype+'_Connectance_table.tsv','w')
  ...
  g=open('../data/randomised_webs/'+nettype+'_LS_table.tsv','w')
  ...
  h=open('../data/randomised_webs/'+nettype+'_LG_table.tsv','w')
...
def main():
  ...
  for nettype in ['SG','GP']:
    ...
    for post_number in range(1,101):
      ...
      # for proportion in [0.9,0.95,0.99]:
        ...
        for sample_number in range(1,101):
          gallers,salixes,links=web_reader(webdir,proportion,post_number,sample_number,nettype)
    ...
    write_summary_file(summary_dict,nettype)
