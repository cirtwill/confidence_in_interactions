...
def write_posterior_webs(random_ints):
  for i in range(1,11):
    outfile=open('../data/randomised_webs/posterior/P'+str(i)+'.web','w')
...
def filter_posterior_webs(postdir,proportion):
  for postweb in os.listdir(postdir):
    ...
    for j in range(1,11):
      ...
      g=open('../data/randomised_webs/detection_filter/'+str(proportion)+'/'+postweb.split('.web')[0]+'_'+str(j)+'.web','w')
      for item in newlist:
